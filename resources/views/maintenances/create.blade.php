<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Bakım Hatırlatması Ekle</h2>
                    ...
                    <form method="POST" action="{{ route('maintenances.store') }}">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-xs text-gray-500">Ne zaman yapılması gerekiyor (yağ, muayene, sigorta vb.)</p>
                                <x-input-error :messages="$errors->get('due_date')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="last_done_date" :value="__('Son Yapıldığı Tarih (opsiyonel)')" />
                                <x-text-input id="last_done_date" class="block mt-1 w-full" type="date" name="last_done_date" :value="old('last_done_date')" />
                            </div>
                            <div>
                                <x-input-label for="last_done_km" :value="__('Son Yapıldığı Km')" />
                                <x-text-input id="last_done_km" class="block mt-1 w-full" type="number" name="last_done_km" :value="old('last_done_km')" min="0" placeholder="Örn: 125000" />
                            </div>
                            <div>
                            ...
                            <div>
                                <x-input-label for="cost" :value="__('Tahmini Maliyet (TL)')" />
