    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Sefer Düzenle') }} - {{ $trip->destination }}
            </h2>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if($trip->status === 'in_progress')
                <div class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-lg text-amber-900 text-sm">
                    Sefer devam ederken bile tüm bilgileri güncelleyebilirsiniz.
                </div>
            @endif
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('trips.update', $trip) }}">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <select id="truck_id" name="truck_id" required
                                    ...
                                    @foreach ($trucks as $t)
                                        <option value="{{ $t->id }}" {{ old('truck_id', $trip->truck_id) == $t->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <x-text-input id="departure_date" class="block mt-1 w-full" type="date" name="departure_date" :value="old('departure_date', $trip->departure_date->format('Y-m-d'))" required />
                            ...
                            <div>
                                ...
                                <input type="text" id="origin" name="origin" list="iller-list" autocomplete="off"
                                    value="{{ old('origin', $trip->origin) }}" placeholder="Yazın veya listeden seçin (opsiyonel)"
                                    class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <div>
                                ...
                                <input type="text" id="destination" name="destination" list="iller-list2" autocomplete="off" required
                                    value="{{ old('destination', $trip->destination) }}" placeholder="Yazın veya listeden seçin"
                                    class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <div>
                                ...
                                <input type="text" id="stopovers" name="stopovers" list="iller-list3" autocomplete="off"
                                    value="{{ old('stopovers', is_array($trip->stopovers) ? implode(', ', $trip->stopovers) : '') }}"
                            ...
                            <div>
                                ...
                                <select id="status" name="status" required
                                    class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="planned" {{ old('status', $trip->status) == 'planned' ? 'selected' : '' }}>Planlandı</option>
                                    <option value="in_progress" {{ old('status', $trip->status) == 'in_progress' ? 'selected' : '' }}>Devam Ediyor</option>
                                    <option value="completed" {{ old('status', $trip->status) == 'completed' ? 'selected' : '' }}>Tamamlandı</option>
                                    <option value="cancelled" {{ old('status', $trip->status) == 'cancelled' ? 'selected' : '' }}>İptal</option>
                                </select>
                            ...
                            <div>
                                ...
                                <select id="customer_id" name="customer_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach($customers ?? [] as $c)
                                        <option value="{{ $c->id }}" {{ old('customer_id', $trip->customer_id) == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <x-input-label for="commission_amount" :value="__('Komisyon Ücreti (TL)')" />
                                <x-text-input id="commission_amount" class="block mt-1 w-full" type="number" name="commission_amount" :value="old('commission_amount', $trip->commission_amount)" step="0.01" min="0" />
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <x-text-input id="revenue_amount" class="block mt-1 w-full" type="number" name="revenue_amount" :value="old('revenue_amount', $trip->revenue_amount)" step="0.01" min="0" />
                                </div>
                                <div>
                                    ...
                                    <select id="payment_status" name="payment_status" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                        @foreach(\App\Models\Trip::PAYMENT_STATUS_LABELS as $v => $l)
                                            <option value="{{ $v }}" {{ old('payment_status', $trip->payment_status ?? 'bekliyor') == $v ? 'selected' : '' }}>{{ $l }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div>
                                ...
                                <x-text-input id="days_stayed" class="block mt-1 w-full" type="number" name="days_stayed" :value="old('days_stayed', $trip->days_stayed)" min="0" placeholder="Seferde kaç gün kalındı" />
                            ...
                            <div class="border-t pt-4 mt-4">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <x-text-input id="start_km" class="block mt-1 w-full" type="number" name="start_km" :value="old('start_km', $trip->start_km)" min="0" placeholder="Örn: 125000" />
                                    </div>
                                    <div>
                                        <x-input-label for="end_km" :value="__('Bitiş Km')" />
                                        <x-text-input id="end_km" class="block mt-1 w-full" type="number" name="end_km" :value="old('end_km', $trip->end_km)" min="0" placeholder="Sefer bitince girilir" />
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div>
                                        ...
                                        <select id="cargo_type" name="cargo_type" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                            ...
                                            @foreach(\App\Models\Trip::CARGO_TYPE_LABELS as $val => $lbl)
                                                <option value="{{ $val }}" {{ old('cargo_type', $trip->cargo_type) == $val ? 'selected' : '' }}>{{ $lbl }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div>
                                        <x-input-label for="load_weight" :value="__('Yük Ağırlığı (ton)')" />
                                        <x-text-input id="load_weight" class="block mt-1 w-full" type="number" name="load_weight" :value="old('load_weight', $trip->load_weight)" step="0.01" min="0" placeholder="Örn: 24" />
                                    </div>
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            <x-input-label for="loading_date" :value="__('Yükleme Tarihi')" />
                                            <x-text-input id="loading_date" class="block mt-1 w-full" type="date" name="loading_date" :value="old('loading_date', $trip->loading_date?->format('Y-m-d'))" />
                                        </div>
                                        <div>
                                            <x-input-label for="unloading_date" :value="__('Boşaltma Tarihi')" />
                                            <x-text-input id="unloading_date" class="block mt-1 w-full" type="date" name="unloading_date" :value="old('unloading_date', $trip->unloading_date?->format('Y-m-d'))" />
                                        </div>
                                    </div>
                                    <div>
                                        <x-input-label for="receiver_name" :value="__('Alıcı / Firma Adı')" />
                                        <x-text-input id="receiver_name" class="block mt-1 w-full" type="text" name="receiver_name" :value="old('receiver_name', $trip->receiver_name)" placeholder="Teslim alan firma" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="notes" name="notes" rows="3"
                                    class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes', $trip->notes) }}</textarea>
                        ...
                        <div class="mt-6 flex flex-wrap gap-4 items-center">
                            ...
                            <a href="{{ route('trips.show', $trip) }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300">
                                {{ __('İptal') }}
                            </a>
                            <form action="{{ route('trips.destroy', $trip) }}" method="POST" class="inline" onsubmit="return confirm('Bu seferi silmek istediğinize emin misiniz?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600 hover:text-red-800 text-sm font-medium">{{ __('Seferi Sil') }}</button>
                            </form>
                        </div>
                    </form>
                </div>
