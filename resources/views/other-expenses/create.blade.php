<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Masraf Ekle') }} - {{ $trip->destination }}
        </h2>
    </x-slot>
    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('other-expenses.store') }}">
                        @csrf
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <select id="category" name="category" required class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    <option value="otel" {{ old('category') == 'otel' ? 'selected' : '' }}>Otel</option>
                                    <option value="yol_geçiş" {{ old('category') == 'yol_geçiş' ? 'selected' : '' }}>Yol Geçiş</option>
                                    <option value="otopark" {{ old('category') == 'otopark' ? 'selected' : '' }}>Otopark</option>
                            ...
                            <div>
                                ...
                                <x-text-input id="amount" class="block mt-1 w-full" type="number" name="amount" :value="old('amount')" step="0.01" min="0" required />
